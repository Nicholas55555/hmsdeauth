...
def quick_sniff_bssid(iface: str, channel: int, target_bssid: str, sniff_time=0.5) -> bool:
...
def supervisor_single_bssid(bssid: str, channel: int, clients: list, iface: str, band: str):
    ...
    try:
        while True:
            ...
            found_on_current = quick_sniff_bssid(iface, current_ch, bssid, sniff_time=0.5)
            if found_on_current:
                ...
                for ch in channels_to_check:
                    if ch == current_ch:
                        continue
                    if quick_sniff_bssid(iface, ch, bssid, sniff_time=0.5):
                        new_channel = ch
                        break
                if new_channel < 0:
...
def main():
    ...
    if band == "2.4":
        ...
        discovered = scan_for_bssids(base_iface, chs_24, dwell=0.5)
        ...
        discovered = scan_for_bssids(base_iface, chs_5, dwell=0.5)
        print("Found BSSIDs, Peforming Reconnaissance")
        time.sleep(2)
        anim_thread.start()
