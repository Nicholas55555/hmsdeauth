...
# ------------------------------------------------------------------------------
# 1) Plain ASCII Banner (instead of Pyfiglet)
# ------------------------------------------------------------------------------
def show_banner():
    banner = text2art("Homing Missile\n Salvo Deauth", font='doom')
    print(banner)
...
"""
    ...
    while not stop_event.is_set():
        # Clear screen + move cursor top-left
...
# ------------------------------------------------------------------------------
# 3) ASCII “Missile Launch” & “Explosion” (One-Off Prints) - No pyfiglet
# ------------------------------------------------------------------------------
def show_missile_launch():
    """
    Just prints ASCII missile volley
    """
    print("\n=================== LAUNCHING MISSILES ===================\n")
    art = r"""
            /\
...
# ------------------------------------------------------------------------------
# Basic monitor-mode, scanning, etc.
# ------------------------------------------------------------------------------
...
def supervisor_single_bssid(bssid: str, channel: int, clients: list, iface: str, band: str):
    ...
    current_ch = channel
    if band == "2.4":
        channels_to_check = get_2ghz_channels(iface)
    else:
        channels_to_check = get_5ghz_channels(iface)
...
# ------------------------------------------------------------------------------
def main():
    # Show a simple ASCII banner (no pyfiglet)
    show_banner()
    ...
    if band == "2.4":
        ...
        print("Found BSSIDs, performing reconnaissance...")
        time.sleep(2)
        anim_thread.start()
    else:
